...
One of the continuous variables we include in our original database is the variable `time_since_invasion`. Let's create a meta-regression model to determine whether time since invasion has an effect on the  effect sizes in our meta-analysis. To do so, we'll use the R package `lme4`

```{r}
library(lme4)
```

Next, we can set up our meta-regression model. We'll use a function from the `lme4` package rather than one from `metafor`.

```{r}
time_since_invasion_moderator <- lmer(yi ~ (1 | focal_organic_system) + organically_managed_years, data = systems_with_more_than_five)
